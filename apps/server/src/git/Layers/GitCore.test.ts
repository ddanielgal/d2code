  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "d2code/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "d2code/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "d2code/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "d2code/tmp-working",
          newBranch: "d2code/feat/session",
        });

        expect(renamed.branch).toBe("d2code/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "d2code/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "d2code/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("d2code/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "d2code/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "d2code/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "d2code/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "d2code/tmp-working" });
        ...
        expect(renamed.branch).toBe("d2code/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "d2code/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
